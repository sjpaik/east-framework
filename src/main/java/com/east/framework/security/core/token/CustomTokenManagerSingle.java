 ...
 */
public class CustomTokenManagerSingle implements CustomTokenManager {
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private Map<UserDetails, CustomTokenInfo> tokens = new HashMap();

	@Override
	public CustomTokenInfo createNewToken(UserDetails userDetails) {
		String token;
		do {
			token = generateToken();
		} while (validUsers.containsKey(token));

		CustomTokenInfo tokenInfo = new CustomTokenInfo(token, userDetails);
	...
	private String generateToken() {
		...
		return new String(Base64.encode(tokenBytes), Charsets.UTF_8);
	}

	@Override
	public void removeUserDetails(UserDetails userDetails) {
		CustomTokenInfo token = tokens.remove(userDetails);
		if (token != null) {
	...
	@Override
	public Collection<CustomTokenInfo> getUserTokens(UserDetails userDetails) {
